...
- **Worker entry:** `vinext/server/app-router-entry` — used directly as `main` in `wrangler.jsonc`. Image optimization is not currently active.
- **Worker config:** [`wrangler.jsonc`](./wrangler.jsonc) configures `ASSETS` binding serves `dist/client`. The `IMAGES` binding is present but unreachable with the current `main`; it was part of an earlier image-optimization attempt.
- **`worker/index.ts`:** Auto-generated by `vinext deploy` when the file is absent. It wires the Cloudflare Images binding into vinext's image optimizer and delegates everything else to `vinext/server/app-router-entry`. The checked-in copy is a snapshot from an earlier experiment with image optimization. It is not the active entry. Whether `vinext deploy` overwrites `wrangler.jsonc main` when it regenerates the file is not yet confirmed — investigate before enabling image optimization.
