...
final class SupabaseAuthenticationRepository extends AuthenticationRepository {
  ...
  Future<Result> signUp({
    ...
    try {
      ...
      return Result.unknown(name: 'SupabaseAuthenticationRepository', error: e, stackTrace: s);
    }
  }
    ...
    try {
      ...
      if (e.code == 'invalid_credentials') {
        ...
        List<Map<String, dynamic>> data = await _supabase
            ...
            .select('id')
            .eq('email', email);
        return Result.failure(data.isEmpty ? const EmailNotFound() : const WrongPassword());
      }
      rethrow;
    } on AuthRetryableFetchException {
      final connected = await hasInternetAccess();
